...
const ManagedByAutofinder = () => {
  ...
  console.log('🎯 ManagedByAutofinder component rendered');
  console.log('🎯 Component state - loading:', loading, 'ads count:', ads.length, 'error:', error);

  useEffect(() => {
    console.log('🚀 ManagedByAutofinder component mounted');
    ...
    const fetchManagedAds = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          // Backend returns all non-deleted ads (same as admin panel)
          // Filter on frontend: Show only active ads with required fields
          // Show pending ads too (they are active, just waiting for approval)
          const validAds = data.filter((ad) => {
            // Relaxed filtering for debugging/live verification
            // Check required fields for display (basic safety)
            const hasRequiredFields = ad.make && ad.model;

            // Relaxed Active Check: Allow true, "true", or undefined (fallback to true for visibility)
            // const isActive = ad.isActive === true; 
            const isActive = true; // FORCE SHOW ALL for now to verify data presence

            // Relaxed Status Check
            const isApproved = true; // FORCE SHOW ALL

            const isValid = hasRequiredFields; // Only check basic fields

            if (!isValid) {
              console.log(`⚠️ Ad filtered out (Strict): ${ad._id}`, {
                make: ad.make,
                model: ad.model
              });
            }
            console.log(`✅ Ad is valid: ${ad._id}`, {
              make: ad.make,
              model: ad.model,
          ...
          console.log(`✅ Total ads from backend: ${data.length}`);
          console.log(`✅ Valid ads after basic filtering: ${validAds.length}`);
          console.log('✅ Valid ads data:', validAds.map(ad => ({
            _id: ad._id,
            make: ad.make,
            model: ad.model,
            price: ad.price,
          ...
          if (validAds.length > 0) {
            setAds(validAds.slice(0, 8)); // Show first 8 ads
            console.log('✅ Ads set successfully:', validAds.slice(0, 8).length);
          } else {
            console.warn('⚠️ No valid ads found after filtering');
            console.warn('⚠️ All ads data for debugging:', data.map(ad => ({
              _id: ad._id,
              make: ad.make,
              model: ad.model,
              price: ad.price,
          ...
          console.warn('⚠️ No ads found or data is not an array');
          console.warn('⚠️ Data received:', data);
          setAds([]);
        }
      } catch (err) {
        console.error('❌ Error fetching managed ads:', err);
        console.error('❌ Error details:', {
          message: err.message,
          stack: err.stack,
          name: err.name
        });
        setError(err.message || 'Failed to fetch managed cars');
        setAds([]);
      } finally {
        setLoading(false);
    ...
    return (
      <div>
        <div className="text-center mb-4">
          <p className="text-gray-600 dark:text-gray-400">Loading managed cars...</p>
        </div>
  ...
  if (error) {
    return (
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-200 dark:border-yellow-800 rounded-lg p-4 sm:p-6 text-center transition-colors">
        <p className="text-yellow-800 dark:text-yellow-400 font-semibold mb-2 text-sm sm:text-base">{t('unableToLoad')} {t('managedByAutofinder').toLowerCase()}</p>
        ...
        <button
          ...
          className="mt-3 sm:mt-4 bg-red-600 dark:bg-red-700 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded text-xs sm:text-sm hover:bg-red-700 dark:hover:bg-red-600"
        >
          Retry
        </button>
      </div>
  ...
  if (ads.length === 0 && !loading) {
    return (
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 sm:p-6 md:p-8 text-center transition-colors">
        <p className="text-gray-600 dark:text-gray-400 mb-2 font-semibold text-sm sm:text-base">No managed cars available at the moment.</p>
        {error && (
        ...
        {ads.map((car) => {
          ...
          return (
            <div
              ...
              className="car-card flex-shrink-0 w-[240px] md:w-[280px] bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-lg dark:shadow-gray-900 overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 group flex flex-col"
            >
              {/* Image Container */}
              <div className="h-40 sm:h-44 md:h-48 bg-gray-200 dark:bg-gray-700 relative overflow-hidden flex-shrink-0">
                <img
                  ...
                  onError={(e) => {
                    ...
                    // Use SVG placeholder instead of external service
                ...
                {/* Managed by Autofinder Badge */}
                <div className="absolute top-2 left-2 bg-red-600 text-white px-2 py-1 rounded text-[10px] sm:text-xs font-bold shadow-lg z-10">
                  <span className="hidden sm:inline">Managed By AutoFinder</span>
                  <span className="sm:hidden">AutoFinder</span>
                </div>

                {/* Featured Badge if applicable */}
                {car.isFeatured && (
                  <div className="absolute top-2 right-2 bg-yellow-500 text-white px-2 py-1 rounded text-[10px] sm:text-xs font-bold shadow-lg">
                    {t('premium')}
                  </div>
                )}
              </div>
              ...
              <div className="p-3 sm:p-4 flex flex-col flex-grow">
                ...
                {/* Location */}
                {car.location && (
                  <div className="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-600 dark:text-gray-400 mb-4">

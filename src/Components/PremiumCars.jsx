...
import { FaCar, FaMapMarkerAlt, FaCalendarAlt, FaCog, FaBolt, FaStar, FaChevronLeft, FaChevronRight } from 'react-icons/fa';
...
const PremiumCars = () => {
  ...
  console.log('🎯 PremiumCars component rendered');

  useEffect(() => {
    console.log('🚀 PremiumCars component mounted');
    ...
    const fetchPremiumCars = async () => {
      try {
        ...
        console.log('🔄 Fetching premium cars from:', endpoint);
        ...
        if (Array.isArray(data) && data.length > 0) {
          // Filter: Show only active, approved premium cars
          const validCars = data.filter((car) => {
            ...
            // Premium cars should be featured/approved
            const isFeatured = car.isFeatured === "Approved" || car.isFeatured === true || car.isFeatured === "approved";
          ...
          console.log(`✅ Valid premium cars after filtering: ${validCars.length}`);
        ...
        setError(err.message || 'Failed to fetch premium cars');
        setCars([]);
      } finally {
        setLoading(false);
  ...
  if (error) {
    return (
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-200 dark:border-yellow-800 rounded-lg p-4 sm:p-6 text-center transition-colors">
        <p className="text-yellow-800 dark:text-yellow-400 font-semibold mb-2 text-sm sm:text-base">{t('unableToLoad')} {t('premiumCars').toLowerCase()}</p>
  ...
  if (cars.length === 0 && !loading) {
    return (
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 sm:p-6 md:p-8 text-center transition-colors">
        <p className="text-gray-600 dark:text-gray-400 mb-2 font-semibold text-sm sm:text-base">No premium cars available at the moment.</p>
        {error && (
          ...
          return (
            ...
            >
              <div className="h-40 sm:h-44 md:h-48 bg-gray-200 dark:bg-gray-700 relative overflow-hidden flex-shrink-0">
                ...
                <div className="absolute top-2 left-2 bg-yellow-500 text-white px-2 py-1 rounded text-[10px] sm:text-xs font-bold shadow-lg z-10 flex items-center gap-1">
                  <FaStar className="text-xs" />
                  {t('premium')}
                </div>

                {car.isBoosted && (
                  <div className="absolute top-2 right-2 bg-red-600 text-white px-2 py-1 rounded text-[10px] sm:text-xs font-bold shadow-lg">
                    BOOSTED
                  </div>
                )}
              </div>

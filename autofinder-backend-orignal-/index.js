...
// When running from backend folder, also load env from project root (for MONGODB_URI etc.)
...
// Resolve modules from project root (models, middleware in root when running from backend folder)
const __root = path.join(__dirname, '..');
const reqRoot = (rel) => require(path.join(__root, rel));
...
const User = reqRoot('models/User');
const InspectionJob = reqRoot('models/InspectionJob');
const Inspector = reqRoot('models/Dashboard/Inspector');
const Free_Ads = reqRoot('models/post_ad/Free_Ads');
const Featured_Ads = reqRoot('models/post_ad/Featured_Ads');
const ListItforyouad = reqRoot('models/ListItforyou/listItforyouad');
const BuyCar = reqRoot('models/Buy_car_for_me/BuyCar');
const Rent_Car = reqRoot('models/post_ad/Rent_Car');
const Inspection = reqRoot('models/car_inspection/Inspection');
const InspectionReport = reqRoot('models/car_inspection/InspectionReport');
const Report = reqRoot('models/Report');
...
try {
  Bike_Ads = reqRoot('models/post_ad/Bike_Ads');
...
try {
  NewCarData = reqRoot('models/NewCarData'); // Exports as New_Car model
...
try {
  AutoStore = reqRoot('models/AutoStore'); // Exports as AutoStoreAd model
...
try {
  DealerPackage = reqRoot('models/DealerPackage');
...
try {
  DealerPackageRequest = reqRoot('models/DealerPackageRequest');
...
try {
  MobilePackagePurchase = reqRoot('models/MobilePackagePurchase');
...
try {
  ChatConversation = reqRoot('models/ChatConversation');
  ChatMessage = reqRoot('models/ChatMessage');
...
let FuelPrice;
try {
  FuelPrice = reqRoot('models/FuelPrice');
...
let Advertising;
try {
  Advertising = reqRoot('models/Advertising');
...
let SupportRequest;
try {
  SupportRequest = reqRoot('models/SupportRequest');
...
let Notification;
try {
  Notification = reqRoot('models/Notification');
...
const securityMiddleware = reqRoot('middleware/security');
...
const requestSecurity = reqRoot('middleware/requestSecurity');
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Allow any localhost or 10.0.2.2 (Android emulator) or 192.168.x.x (local network)
...
// Middleware to add CORS headers for static files (allow mobile app / any origin for /uploads)
const staticCorsMiddleware = (req, res, next) => {
  ...
  // For /uploads (images), allow any origin so mobile app and webviews can load images
...
// Normalize image paths: strip all whitespace so mobile app can load images (fixes "image1-17 70638831685" style paths)
function normalizeImagePath(str) {
  if (str == null || typeof str !== 'string') return str;
  return str.replace(/[\s\u00A0\u2000-\u200B\u202F\u205F\u3000\uFEFF]+/g, '').trim() || str;
}
function normalizeAdImagePaths(ad) {
  if (!ad || typeof ad !== 'object') return;
  for (let i = 1; i <= 20; i++) {
    const key = 'image' + i;
    if (ad[key]) ad[key] = normalizeImagePath(ad[key]);
  }
  if (Array.isArray(ad.images)) {
    ad.images = ad.images.map(normalizeImagePath).filter(Boolean);
  }
}
...
app.use('/uploads', staticCorsMiddleware, express.static(path.join(__dirname, 'uploads'), {
  ...
  setHeaders: (res, filePath) => {
    // Allow any origin for image requests so mobile app can load /uploads/... images
    res.setHeader('Access-Control-Allow-Origin', '*');
  }
}));
...
if (fs.existsSync(publicPath)) {
  app.use('/public', staticCorsMiddleware, express.static(publicPath, {
    ...
    lastModified: true,
  setHeaders: (res) => {
    // Allow any origin so mobile app can load /uploads/... images
    res.setHeader('Access-Control-Allow-Origin', '*');
  }
}));
}
...
app.get("/featured_ads/public", securityMiddleware.sanitizePublicResponse, async (req, res) => {
  ...
  try {
    ...
    const normalizedAds = filteredAds.map((ad) => {
      ...
      // Strip spaces from image paths so app can load images (fixes "image1-17 70638831685" style paths)
      normalizeAdImagePaths(ad);
...
app.get("/admin/premium-bike-ads", authenticateToken, async (req, res) => {
  try {
    ...
    // Fetch all premium bike ads (only bikes with isFeatured set to premium statuses)
    // Premium bikes have isFeatured: "Pending", "Approved", or "Rejected"
    // Free bikes (without isFeatured or with null/empty isFeatured) should NOT be included here
    const premiumBikeAds = await Bike_Ads.find({ 
      isDeleted: { $ne: true },
      isFeatured: { $in: ['Pending', 'Approved', 'Rejected'] } // Only bikes with premium status
    })
    .select('_id userId title make model variant year price location adCity registrationCity kmDriven fuelType engineCapacity enginetype description features preferredContact isActive isDeleted isSold isFeatured paymentStatus packageId packageName packagePrice paymentAmount invoiceImage dateAdded featuredExpiryDate validityDays approvedAt views favoritedBy image1 image2 image3 image4 image5 image6 image7 image8 image9 image10 image11 image12 image13 image14 image15 image16 image17 image18 image19 image20')
    .populate('userId', 'name email phone')
    .sort({ dateAdded: -1 })
    .lean();
    
    // Calculate validity days and expiry date based on package name
    const getValidityDays = (packageName) => {
      if (!packageName) return null;
      const packageLower = String(packageName).toLowerCase().trim();
      if (packageLower.includes('starter')) return 7;
      if (packageLower.includes('executive')) return 30;
      if (packageLower.includes('value')) return 15;
      return null;
    };
    
    // Process ads to add validity days and expiry date
    const processedAds = (premiumBikeAds || []).map((ad) => {
      const processed = { ...ad };
      
      // Calculate validity days from package name
      if (!processed.validityDays && processed.packageName) {
        processed.validityDays = getValidityDays(processed.packageName);
      }
      
      // Calculate expiry date if validity days exist
      if (processed.validityDays && !processed.featuredExpiryDate) {
        // Use approvedAt if available, otherwise use dateAdded
        const startDate = processed.approvedAt 
          ? new Date(processed.approvedAt)
          : (processed.dateAdded ? new Date(processed.dateAdded) : new Date());
        
        if (!isNaN(startDate.getTime())) {
          const expiryDate = new Date(startDate);
          expiryDate.setDate(expiryDate.getDate() + processed.validityDays);
          processed.featuredExpiryDate = expiryDate.toISOString();
        }
      }
      
      // Ensure invoiceImage is included (payment receipt)
      // invoiceImage should already be in the response from the query
      
      return processed;
    });
    
    console.log(`✅ /admin/premium-bike-ads: Found ${processedAds.length} premium bike ads`);
    if (processedAds.length > 0) {
      console.log(`🔍 Sample premium bike ad:`, {
        id: processedAds[0]._id,
        make: processedAds[0].make,
        model: processedAds[0].model,
        isFeatured: processedAds[0].isFeatured,
        paymentStatus: processedAds[0].paymentStatus,
        packageName: processedAds[0].packageName,
        validityDays: processedAds[0].validityDays,
        featuredExpiryDate: processedAds[0].featuredExpiryDate,
        invoiceImage: processedAds[0].invoiceImage
      });
    }
...
app.patch("/admin/premium-bikes/:id/approve", authenticateToken, async (req, res) => {
  try {
    ...
    // Get the ad first to check package name
    const existingAd = await Bike_Ads.findById(id).lean();
    if (!existingAd) {
      return res.status(404).json({ success: false, message: "Bike ad not found." });
    }

    // Calculate validity days based on package name
    const getValidityDays = (packageName) => {
      if (!packageName) return null;
      const packageLower = String(packageName).toLowerCase().trim();
      if (packageLower.includes('starter')) return 7;
    ...
    const validityDays = getValidityDays(existingAd.packageName);
    
    // Calculate expiry date from approval date
    let featuredExpiryDate = null;
    if (validityDays) {
      const approvedAt = new Date();
      featuredExpiryDate = new Date(approvedAt);
      featuredExpiryDate.setDate(featuredExpiryDate.getDate() + validityDays);
    }
    ...
    // Add validity days and expiry date if package name exists
    if (validityDays) {
      updateData.validityDays = validityDays;
    }
    if (featuredExpiryDate) {
      updateData.featuredExpiryDate = featuredExpiryDate;
    }
...
app.get("/admin/free-bike-ads", authenticateToken, async (req, res) => {
  try {
    ...
    // Free bikes are bikes where isFeatured is NOT set to premium statuses
    // Premium bikes have isFeatured: "Pending", "Approved", or "Rejected"
    // CRITICAL: Exclude ALL bikes where isFeatured is one of the premium statuses
    // MongoDB $nin will match: field doesn't exist, field is null, or field value is not in array
    const freeBikeAds = await Bike_Ads.find({ 
      isDeleted: { $ne: true },
      isFeatured: { $nin: ['Pending', 'Approved', 'Rejected'] } // Exclude premium statuses (also matches null/undefined/empty)
    })
    .populate('userId', 'name email phone')
    .sort({ dateAdded: -1 })
    .lean();
    
    console.log(`✅ /admin/free-bike-ads: Found ${freeBikeAds.length} free bike ads`);
    if (freeBikeAds.length > 0) {
      console.log(`🔍 Sample free bike ad:`, {
        id: freeBikeAds[0]._id,
        make: freeBikeAds[0].make,
        model: freeBikeAds[0].model,
        isFeatured: freeBikeAds[0].isFeatured,
        isPaidAd: freeBikeAds[0].isPaidAd,
        paymentStatus: freeBikeAds[0].paymentStatus
      });
    }
...
// ==================== ADMIN DELETE BIKE ADS ENDPOINT ====================
// Delete bike ad (requires Admin/SuperAdmin authentication)
// Works for both premium and free bike ads
app.delete("/admin/bike_ads/:id", authenticateToken, async (req, res) => {
  try {
    // Check if user is Admin or SuperAdmin (case-insensitive)
    const userType = String(req.userType || '').toLowerCase();
    if (userType !== 'admin' && userType !== 'superadmin') {
    ...
    if (!id) {
      return res.status(400).json({
        success: false,
        message: "Bike ad ID is required"
      });
    }
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({
        success: false,
        message: "Invalid bike ad ID format"
      });
    }
    ...
    if (!Bike_Ads) {
      return res.status(503).json({
        success: false,
        message: "Bike_Ads model is not loaded"
      });
    }

    // Check if bike ad exists
    const bikeAd = await Bike_Ads.findById(id).lean();
    if (!bikeAd) {
      return res.status(404).json({
        success: false,
        message: "Bike ad not found"
      });
    }

    if (bikeAd.isDeleted === true) {
      return res.status(400).json({
        success: false,
        message: "Bike ad is already deleted"
      });
    }

    // Soft delete: set isDeleted to true
    await Bike_Ads.findByIdAndUpdate(
      id,
      {
        isDeleted: true,
        isActive: false,
    ...
    console.log(`✅ Bike ad ${id} deleted by admin ${req.userId}`);
    res.json({
      success: true,
    ...
    console.error('❌ Error deleting bike ad:', error);
    res.status(500).json({
      success: false,
      message: "Error deleting bike ad",
      error: process.env.NODE_ENV !== 'production' ? error.message : undefined
    });
  }
});
...
app.get("/all_ads/:id", securityMiddleware.sanitizePublicResponse, async (req, res) => {
  try {
    ...
    for (const { model, source } of collections) {
      let doc;
      if (source === 'featured_ads' || source === 'bike_ads') {
        // For Featured_Ads and Bike_Ads, fetch without .lean() first to preserve ObjectId (especially userId)
        // CRITICAL: Don't use .lean() - it can cause ObjectId to lose its methods
        doc = await model.findOne({ _id: id, isDeleted: { $ne: true } });
        if (doc) {
          // Convert to plain object but preserve userId ObjectId structure
          // toObject() preserves ObjectId instances, unlike lean() which converts them to plain objects
          doc = doc.toObject ? doc.toObject({ getters: false, virtuals: false }) : doc;
          
          // DEBUG: Log userId after toObject() for bike_ads
          if (source === 'bike_ads' && doc.userId) {
            console.log(`🔍 /all_ads/:id - After toObject(), userId:`, {
              userId: doc.userId,
              userIdType: typeof doc.userId,
              userIdConstructor: doc.userId?.constructor?.name,
              hasToString: typeof doc.userId?.toString === 'function',
              toStringResult: doc.userId?.toString ? doc.userId.toString() : 'N/A'
            });
          }
        }
      } else {
      ...
      if (doc) {
        // DEBUG: Log raw doc before serialization (for bike_ads only)
        if (source === 'bike_ads') {
          console.log(`🔍 /all_ads/:id - Raw doc userId before serialization:`, {
            userId: doc.userId,
            userIdType: typeof doc.userId,
            userIdConstructor: doc.userId?.constructor?.name,
            userIdKeys: typeof doc.userId === 'object' ? Object.keys(doc.userId || {}) : 'N/A',
            userIdString: doc.userId?.toString ? doc.userId.toString() : String(doc.userId),
            adId: doc._id,
            make: doc.make,
            model: doc.model
          });
        ...
        // Serialize userId - FIXED: Properly handle ObjectId (especially for Featured_Ads and Bike_Ads)
        if (serialized.userId) {
          if (typeof serialized.userId === 'string') {
            ...
            // CRITICAL: Check if it's an ObjectId instance FIRST (before checking keys)
            // ObjectId instances don't have enumerable keys, so Object.keys() returns []
            const mongoose = require('mongoose');
            let userIdStr = null;
            let isObjectIdInstance = false;
            
            // Method 1: Try toString() first - if it returns a valid 24-char hex string, it's an ObjectId
            if (serialized.userId.toString && typeof serialized.userId.toString === 'function') {
              try {
                userIdStr = serialized.userId.toString();
                // Validate: must be 24 hex characters (valid ObjectId format)
                if (userIdStr && userIdStr !== '[object Object]' && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                  isObjectIdInstance = true;
                  if (source === 'bike_ads') {
                    console.log(`🔍 /all_ads/:id - Detected ObjectId via toString(): ${userIdStr}`);
                  }
                } else if (source === 'bike_ads') {
                  console.log(`⚠️ /all_ads/:id - toString() result invalid: ${userIdStr}, length: ${userIdStr?.length}`);
                }
              } catch (e) {
                if (source === 'bike_ads') {
                  console.log(`❌ /all_ads/:id - toString() failed:`, e.message);
                }
              }
            }
            
            // Method 2: Check constructor name (ObjectId instances have constructor.name === 'ObjectId')
            if (!isObjectIdInstance && serialized.userId.constructor && serialized.userId.constructor.name === 'ObjectId') {
              try {
                userIdStr = serialized.userId.toString();
                if (userIdStr && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                  isObjectIdInstance = true;
            ...
            // Method 3: Check instanceof
            if (!isObjectIdInstance) {
              try {
                if (serialized.userId instanceof mongoose.Types.ObjectId) {
                  userIdStr = serialized.userId.toString();
                  if (userIdStr && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                    isObjectIdInstance = true;
                  }
            ...
            if (isObjectIdInstance && userIdStr) {
              // It's an ObjectId instance - convert to string directly
              serialized.userId = userIdStr;
              console.log(`✅ /all_ads/:id - Converted ObjectId userId to string: ${userIdStr} (source: ${source})`);
            } else if (serialized.userId._id) {
              // Has nested _id - try to extract (populated user object)
              try {
                ...
                } else if (typeof nestedId === 'string' && nestedId.length === 24 && /^[0-9a-fA-F]{24}$/.test(nestedId)) {
                  // Nested _id is already a string
                  serialized.userId = nestedId;
                } else {
                  serialized.userId = null;
                }
              } catch (e) {
              ...
              // Not an ObjectId instance - check for populated user object or empty object
              const keys = Object.keys(serialized.userId);
              if (keys.length === 0) {
                // Empty object - silently set to null (no logging for performance)
                serialized.userId = null;
              } else {
                // Has keys but not a valid ObjectId - might be a populated user object
                // Try to extract _id from the object if it exists
                if (serialized.userId._id) {
                  try {
                    const extractedId = serialized.userId._id.toString ? serialized.userId._id.toString() : String(serialized.userId._id);
                    if (extractedId && extractedId.length === 24 && /^[0-9a-fA-F]{24}$/.test(extractedId)) {
                      serialized.userId = extractedId;
                    } else {
                      serialized.userId = null;
                    }
                  } catch (e) {
          ...
          // userId is missing - log for bike_ads to debug
          if (source === 'bike_ads') {
            console.log(`⚠️ /all_ads/:id - userId is null/undefined for bike ad:`, {
              adId: serialized._id,
        ...
        // Strip spaces from image paths so app can load images
        normalizeAdImagePaths(serialized);
...
// Update ad by ID (auth) - for admin panel Managed by AutoFinder toggle and general ad editing
app.put("/all_ads/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    if (!id || !require('mongoose').Types.ObjectId.isValid(id)) {
      return res.status(400).json({ success: false, message: "Invalid ad ID" });
    ...
    // CRITICAL: Log raw request data first
    console.log('📥 PUT /all_ads/:id - Raw req.body:', req.body);
    console.log('📥 PUT /all_ads/:id - Raw req.body type:', typeof req.body);
    console.log('📥 PUT /all_ads/:id - Content-Type header:', req.headers['content-type']);
    console.log('📥 PUT /all_ads/:id - All headers:', JSON.stringify(req.headers, null, 2));
    
    // Try to parse body if it's a string (sometimes bodyParser doesn't parse it)
    let body = req.body || {};
    ...
    // If body is still empty, try to read from raw body
    if (!body || Object.keys(body).length === 0) {
      console.warn('⚠️ Body is empty, checking req.rawBody or req.text');
      if (req.rawBody) {
        try {
    ...
    // Debug: Always log incoming request body to diagnose issues
    console.log('📝 PUT /all_ads/:id - Final body:', JSON.stringify(body, null, 2));
    console.log('📝 PUT /all_ads/:id - Final body type:', typeof body);
    console.log('📝 PUT /all_ads/:id - Final body keys:', Object.keys(body));
    console.log('📝 PUT /all_ads/:id - Final body length:', Object.keys(body).length);
    
    // Handle isActive field (for admin panel toggle)
    if (typeof body.isActive === 'boolean') {
      updates.isActive = body.isActive;
      if (body.isActive) updates.adStatus = 'approved';
      else updates.adStatus = 'pending';
    }
    
    // Handle general ad fields (for EditAdScreen)
    const allowedFields = [
      'title', 'price', 'description', 'location', 'make', 'model', 'year', 
      'kmDriven', 'fuelType', 'transmission', 'bodyType', 'color', 'variant',
      'registrationCity', 'bodyColor', 'engineCapacity', 'assembly', 'features',
      'preferredContact', 'company', 'mileage', 'engineType', 'fuel', 'fueltype',
      'fuel_type', 'km', 'kilometer', 'adCity'
    ];
    
    // Add allowed fields to updates - also accept any field that exists in body (more flexible)
    const allBodyKeys = Object.keys(body);
    console.log('📝 All body keys:', allBodyKeys);
    
    // First, process allowed fields
      ...
      if (body[field] !== undefined && body[field] !== null) {
        ...
        if (field === 'features') {
          if (Array.isArray(body[field])) {
            ...
            console.log(`✅ Added ${field}:`, body[field]);
          }
        } else {
          // Convert numeric fields - allow 0 values too
          if (['price', 'year', 'kmDriven', 'mileage', 'km', 'kilometer', 'engineCapacity'].includes(field)) {
            const numValue = typeof body[field] === 'string' ? parseInt(body[field], 10) : Number(body[field]);
            if (!isNaN(numValue)) {
              updates[field] = numValue;
              console.log(`✅ Added numeric ${field}:`, numValue);
            }
          } else {
            // String fields - allow empty strings but trim them
            const strValue = typeof body[field] === 'string' ? body[field].trim() : String(body[field]);
            if (strValue !== '') {
              updates[field] = strValue;
              console.log(`✅ Added string ${field}:`, strValue);
            }
          }
        }
      }
    ...
    // Also accept any other fields that might be in the body (for flexibility)
    allBodyKeys.forEach(field => {
      // Skip if already processed or if it's a special field
      if (allowedFields.includes(field) || field === 'isActive' || field === '_id' || field === 'id') {
        return;
      }
      
      // Accept any other field that has a value
      if (body[field] !== undefined && body[field] !== null && body[field] !== '') {
        updates[field] = body[field];
        console.log(`✅ Added additional field ${field}:`, body[field]);
      }
    });
    
    // Debug: Always log what will be updated
    console.log('📝 PUT /all_ads/:id - Updates to apply:', JSON.stringify(updates, null, 2));
    console.log('📝 PUT /all_ads/:id - Updates count:', Object.keys(updates).length);
    
    // FALLBACK: If no updates found but body has fields, accept all non-system fields
    if (Object.keys(updates).length === 0 && Object.keys(body).length > 0) {
      console.warn('⚠️ No updates from allowedFields, trying fallback - accepting all body fields');
      const systemFields = ['_id', 'id', 'userId', 'sellerId', 'dateAdded', 'createdAt', 'updatedAt', 'isDeleted', 'isSold', 'views', 'favoritedBy'];
      Object.keys(body).forEach(field => {
        if (!systemFields.includes(field) && body[field] !== undefined && body[field] !== null) {
          // Skip empty strings for non-numeric fields
          if (typeof body[field] === 'string' && body[field].trim() === '') {
            return;
          }
          updates[field] = body[field];
          console.log(`✅ Fallback: Added field ${field}:`, body[field]);
        }
      });
    }
    
    if (Object.keys(updates).length === 0) {
      console.error('❌ PUT /all_ads/:id - No valid fields to update');
      console.error('❌ Received body:', JSON.stringify(body, null, 2));
      console.error('❌ Received body keys:', Object.keys(body));
      console.error('❌ Received body values:', Object.values(body));
      console.error('❌ Allowed fields:', allowedFields);
      return res.status(400).json({ 
        success: false, 
        message: "No valid fields to update (e.g. isActive, title, price, etc.)",
    ...
    if (process.env.NODE_ENV !== 'production') console.error('PUT all_ads/:id error:', error);
    return res.status(500).json({
      success: false,
...
app.get("/all_ads", securityMiddleware.sanitizePublicResponse, async (req, res) => {
  ...
  try {
    ...
    const queryPromise = Promise.all(
      collections.map(async ({ model, source, query: collectionQuery }) => {
        ...
        if (source === 'featured_ads') {
          ...
          // Debug: Log query for premium cars
          if (process.env.NODE_ENV !== 'production') {
            console.log(`🔍 Premium cars query (featured_ads):`, JSON.stringify(finalQuery, null, 2));
          }
        }
        ...
        // Free_ads: no isActive filter so free ads show immediately in Used Cars
        if (source === 'free_ads') {
        ...
        // Debug: Log how many premium cars were found
        if (source === 'featured_ads' && process.env.NODE_ENV !== 'production') {
          console.log(`⭐ Featured_Ads query returned ${list.length} premium cars`);
          if (list.length > 0) {
            console.log(`   Sample: ${list[0].make} ${list[0].model} - isFeatured: ${list[0].isFeatured}, isActive: ${list[0].isActive}`);
          }
        }
        ...
        const enrichedList = filtered.map(doc => {
          ...
          if (source === 'featured_ads') {
            ...
            // So app shows approved premium in Used Cars even if paymentStatus was pending
            if (enriched.isFeatured === 'Approved') {
              enriched.paymentStatus = enriched.paymentStatus || 'verified';
            }
          } else if (source === 'free_ads') {
            // FIXED: Always set modelType to 'Free' for free_ads, ignore doc.modelType (may be incorrect)
            enriched.modelType = 'Free';
        ...
        // Debug: Log enriched premium cars count
        if (source === 'featured_ads' && process.env.NODE_ENV !== 'production') {
          console.log(`✅ After enrichment: ${enrichedList.length} premium cars (from ${filtered.length} filtered, ${list.length} raw)`);
        }
        
        return enrichedList;
        } catch (err) {
          if (process.env.NODE_ENV !== 'production') {
            console.error('/all_ads query failed for', source, ':', err.message);
          }
          return [];
        }
      })
    ...
    clearTimeout(timeout);
    // Debug: log count per source so free_ads can be verified (e.g. free_ads: 0 means query issue)
    if (process.env.NODE_ENV !== 'production' && Array.isArray(results) && results.length === collections.length) {
      const counts = collections.map((c, i) => c.source + ':' + (results[i]?.length ?? 0)).join(', ');
      console.log('/all_ads counts:', counts);
    }
    ...
    const all = results.flat()
      ...
      .sort((a, b) => {
        // Used Cars order: 1) Managed by AutoFinder, 2) Premium (approved), 3) Free, then others; within same group newest first
        const priority = (ad) => {
          if (ad.adSource === 'list_it_for_you_ad') return 0;
          if (ad.adSource === 'featured_ads') return 1;
          if (ad.adSource === 'free_ads') return 2;
          return 3; // bike_ads, new_cars, rent_car
        };
        const pA = priority(a);
        const pB = priority(b);
    ...
    // Strip spaces from image paths so app can load images
    all.forEach(normalizeAdImagePaths);
    res.json(all);
  } catch (error) {
    clearTimeout(timeout);
...
// POST list_it_for_you_ad/update-status - body: { id, isActive } (for admin panel; no :id in URL so proxy-friendly)
app.post("/list_it_for_you_ad/update-status", authenticateToken, async (req, res) => {
  try {
    const { id, isActive } = req.body || {};
    if (!id || !require('mongoose').Types.ObjectId.isValid(id)) {
      return res.status(400).json({ success: false, message: "Invalid or missing id in body." });
    }
    if (typeof isActive !== 'boolean') {
      return res.status(400).json({ success: false, message: "isActive (boolean) required in body." });
    }
    const ad = await ListItforyouad.findById(id);
    if (!ad) return res.status(404).json({ success: false, message: "Ad not found." });
    if (ad.isDeleted) return res.status(400).json({ success: false, message: "Ad is deleted." });
    const updated = await ListItforyouad.findByIdAndUpdate(
      id,
      { isActive, adStatus: isActive ? 'approved' : 'pending' },
      { new: true }
    ).lean();
    return res.json(updated);
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') console.error('POST list_it_for_you_ad/update-status error:', error);
    return res.status(500).json({ success: false, message: "Error updating ad.", error: process.env.NODE_ENV !== 'production' ? error.message : undefined });
  }
});
...
// PUT list_it_for_you_ad/:id - update ad (e.g. toggle isActive) - Admin/auth
app.put("/list_it_for_you_ad/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    if (!id || !require('mongoose').Types.ObjectId.isValid(id)) {
      return res.status(400).json({ success: false, message: "Invalid ad ID." });
    ...
    if (typeof body.isActive === 'boolean') {
      updates.isActive = body.isActive;
      updates.adStatus = body.isActive ? 'approved' : 'pending';
    }
    if (Object.keys(updates).length === 0) {
      return res.status(400).json({ success: false, message: "No valid fields to update." });
    }
    const updated = await ListItforyouad.findByIdAndUpdate(id, updates, { new: true }).lean();
    res.json(updated);
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') console.error('PUT list_it_for_you_ad error:', error);
    res.status(500).json({ success: false, message: "Error updating ad.", error: process.env.NODE_ENV !== 'production' ? error.message : undefined });
  }
});

// Shared handler for list_it_for_you toggle-status (used by PUT and POST)
const listItForYouToggleStatus = async (req, res) => {
  try {
    const { id } = req.params;
    if (!id || !require('mongoose').Types.ObjectId.isValid(id)) {
      return res.status(400).json({ success: false, message: "Invalid ad ID." });
    ...
    const updated = await ListItforyouad.findByIdAndUpdate(
      id,
      { isActive: newActive, adStatus: newActive ? 'approved' : 'pending' },
      { new: true }
    ).lean();
    res.json(updated);
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') console.error('list_it_for_you_ad toggle-status error:', error);
    res.status(500).json({ success: false, message: "Error toggling ad status.", error: process.env.NODE_ENV !== 'production' ? error.message : undefined });
  }
};

app.put("/list_it_for_you_ad/:id/toggle-status", authenticateToken, listItForYouToggleStatus);
// POST fallback for proxies/servers that block PUT
app.post("/list_it_for_you_ad/:id/toggle-status", authenticateToken, listItForYouToggleStatus);

// PATCH list_it_for_you_ad/:id - same as PUT (for proxies that block PUT)
app.patch("/list_it_for_you_ad/:id", authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    if (!id || !require('mongoose').Types.ObjectId.isValid(id)) {
      return res.status(400).json({ success: false, message: "Invalid ad ID." });
    ...
    if (process.env.NODE_ENV !== 'production') console.error('PATCH list_it_for_you_ad error:', error);
    res.status(500).json({ success: false, message: "Error updating ad.", error: process.env.NODE_ENV !== 'production' ? error.message : undefined });
  }
});
...
app.post("/bike_ads", uploadBikeAdsImages, multerErrorHandler, async (req, res) => {
  try {
    ...
    if (!userId) {
      console.error('❌ POST /bike_ads: userId is missing from request body');
      return res.status(400).json({ success: false, message: "userId is required." });
    }
    
    const mongoose = require('mongoose');
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      console.error('❌ POST /bike_ads: Invalid userId format:', userId);
      return res.status(400).json({ success: false, message: "Invalid userId." });
    }
    
    console.log('✅ POST /bike_ads: Valid userId received:', userId);
    const title = (b.title || '').trim();
    const make = (b.make || '').trim();
    const model = (b.model || '').trim();
    const location = (b.location || '').trim();
    ...
    // CRITICAL: Ensure userId is properly converted to ObjectId
    let userIdObjectId;
    try {
      userIdObjectId = new mongoose.Types.ObjectId(userId);
      console.log('✅ userId converted to ObjectId:', userIdObjectId.toString());
    } catch (e) {
    ...
    // Premium bike ad: when user posts from "PostBikeAdFeatured" flow, app sends isFeatured=true or packageId/packageName
    console.log('🔍 POST /bike_ads - Checking premium status:', {
      isFeatured: b.isFeatured,
      isFeaturedType: typeof b.isFeatured,
      isPaidAd: b.isPaidAd,
    ...
    const hasPackageId = b.packageId && String(b.packageId).trim() !== '';
    const hasPackageName = b.packageName && String(b.packageName).trim() !== '';
    const isFeaturedTrue = b.isFeatured === true || (typeof b.isFeatured === 'string' && b.isFeatured.toLowerCase() === 'true');
    const isPaidAdTrue = b.isPaidAd === true || (typeof b.isPaidAd === 'string' && b.isPaidAd.toLowerCase() === 'true');
    
    const isPremiumAd = isFeaturedTrue || isPaidAdTrue || hasPackageId || hasPackageName;
    ...
    if (isPremiumAd) {
      doc.isFeatured = 'Pending';
      doc.paymentStatus = (b.paymentStatus || 'pending').toString().trim().toLowerCase() || 'pending';
      if (b.packageId) doc.packageId = String(b.packageId).trim();
      if (b.packageName) doc.packageName = String(b.packageName).trim();
      if (b.packagePrice != null && b.packagePrice !== '') doc.packagePrice = parseFloat(b.packagePrice) || 0;
      console.log('✅ Bike ad created as PREMIUM (isFeatured: Pending) – will show in Admin Premium Bikes');
      console.log('📋 Premium ad doc:', {
      ...
      console.log('ℹ️ Bike ad created as FREE (no isFeatured set) – will show in Free Bikes Ads');
    }
    const ad = new Bike_Ads(doc);
    await ad.save();
    
    // Log what was actually saved
    const savedAd = await Bike_Ads.findById(ad._id).lean();
    console.log('💾 Bike ad saved with ID:', ad._id.toString());
    console.log('💾 Saved ad details:', {
      id: savedAd?._id,
      title: savedAd?.title,
      make: savedAd?.make,
      model: savedAd?.model,
      userId: savedAd?.userId,
      userIdType: typeof savedAd?.userId,
      userIdString: savedAd?.userId ? String(savedAd?.userId) : 'N/A',
      isFeatured: savedAd?.isFeatured,
      paymentStatus: savedAd?.paymentStatus,
      packageId: savedAd?.packageId,
      packageName: savedAd?.packageName,
      packagePrice: savedAd?.packagePrice
    });
    
    // CRITICAL: Verify userId was saved correctly
    if (!savedAd?.userId) {
      console.error('❌ CRITICAL: userId was NOT saved to database!', {
        adId: ad._id.toString(),
        expectedUserId: userId,
        savedUserId: savedAd?.userId
      });
    } else {
      console.log('✅ userId saved correctly:', {
        adId: ad._id.toString(),
        userId: String(savedAd.userId),
        userIdType: typeof savedAd.userId
      });
    }
...
app.get("/bike_ads/public", securityMiddleware.sanitizePublicResponse, async (req, res) => {
  ...
  try {
    ...
    // CRITICAL: Do NOT use .populate() - we want userId as ObjectId, not populated object
    // CRITICAL: Do NOT use .lean() - use .toObject() to preserve ObjectId structure for proper serialization
    // CRITICAL: Explicitly select userId to ensure it's included
    const queryPromise = Bike_Ads.find({
      isDeleted: { $ne: true },
      isFeatured: { $nin: ['Pending', 'Rejected'] }
    })
      .select('_id userId sellerId postedBy title make model variant year price location adCity registrationCity kmDriven fuelType engineCapacity enginetype description features preferredContact isActive isDeleted isSold isFeatured paymentStatus packageId packageName packagePrice invoiceImage dateAdded featuredExpiryDate views favoritedBy image1 image2 image3 image4 image5 image6 image7 image8 image9 image10 image11 image12 image13 image14 image15 image16 image17 image18 image19 image20')
      .sort({ dateAdded: -1 })
      .limit(500) // Limit to 500 records
      .maxTimeMS(15000) // MongoDB timeout: 15 seconds
      .exec()
      .then(docs => {
        // Convert to plain objects but preserve ObjectId structure
    ...
    try {
      ...
      // CRITICAL: Pre-process userId conversion BEFORE normalization
      // Convert userId from ObjectId to string immediately after fetch
      bikeAds = (bikeAds || []).map((ad) => {
        const processed = { ...ad };
        
        // DEBUG: Log raw userId before processing
        if (processed.userId !== undefined && processed.userId !== null) {
          console.log('🔍 Raw userId before pre-processing:', {
            userId: processed.userId,
            userIdType: typeof processed.userId,
            userIdConstructor: processed.userId?.constructor?.name,
            userIdKeys: typeof processed.userId === 'object' ? Object.keys(processed.userId) : 'N/A',
            userIdString: String(processed.userId),
            make: processed.make,
            model: processed.model
          });
        }
        
        // Convert userId ObjectId to string immediately
        if (processed.userId !== undefined && processed.userId !== null) {
          const mongoose = require('mongoose');
          if (typeof processed.userId === 'object') {
            // CRITICAL: Check if it's an ObjectId instance FIRST (before checking keys)
            // ObjectId instances don't have enumerable keys, so Object.keys() returns []
            // Strategy: Check constructor name FIRST (most reliable), then toString()
            let userIdStr = null;
            let isObjectIdInstance = false;
            
            // CRITICAL FIX: Check constructor name FIRST - this is the most reliable way
            const constructorName = processed.userId.constructor?.name;
            
            // Method 1: Check constructor name FIRST (most reliable indicator)
            if (constructorName === 'ObjectId') {
              try {
                userIdStr = processed.userId.toString();
                // Validate: must be 24 hex characters (valid ObjectId format)
                if (userIdStr && userIdStr !== '[object Object]' && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                  isObjectIdInstance = true;
                  processed.userId = userIdStr; // Convert immediately
                  console.log('✅ Converted ObjectId userId to string (constructor check):', processed.userId);
                } else {
                  console.log(`⚠️ Constructor check passed but validation failed: ${userIdStr}, length: ${userIdStr?.length}`);
                }
              } catch (e) {
                console.log('❌ toString() failed after constructor check:', e.message);
              }
            }
            
            // Method 2: Try toString() directly if constructor check didn't work
            if (!isObjectIdInstance && processed.userId.toString && typeof processed.userId.toString === 'function') {
              try {
                userIdStr = processed.userId.toString();
                // Validate: must be 24 hex characters (valid ObjectId format)
                if (userIdStr && userIdStr !== '[object Object]' && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                  isObjectIdInstance = true;
                  processed.userId = userIdStr; // Convert immediately
                  console.log('✅ Converted ObjectId userId to string (toString check):', processed.userId);
                }
              } catch (e) {
            ...
            // Method 3: Check instanceof as final backup
            if (!isObjectIdInstance) {
              try {
                if (processed.userId instanceof mongoose.Types.ObjectId) {
                  userIdStr = processed.userId.toString();
                  if (userIdStr && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                    isObjectIdInstance = true;
                    processed.userId = userIdStr;
                    console.log('✅ Converted ObjectId userId to string (instanceof check):', processed.userId);
                  }
                }
              } catch (e) {
                console.log('❌ instanceof check failed:', e.message);
              }
            }
            
            // Only check for empty object if ObjectId detection failed
            if (!isObjectIdInstance) {
              console.log('⚠️ ObjectId detection failed, checking for empty object or populated user. Constructor:', constructorName);
              // It's not an ObjectId instance - check for populated user object or empty object
              const keys = Object.keys(processed.userId);
              
              // Check if it's an empty object
              if (keys.length === 0) {
                console.warn('⚠️ Empty userId object found, setting to null');
                processed.userId = null;
              } else {
                // Check if it's a populated user object (has _id property)
                if (processed.userId._id !== undefined) {
                  const nestedId = processed.userId._id;
                  if (nestedId && nestedId.toString && typeof nestedId.toString === 'function') {
                    const idStr = nestedId.toString();
                    if (idStr && idStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(idStr)) {
                      processed.userId = idStr;
                      console.log('✅ Extracted userId from populated _id:', processed.userId);
                    } else {
                      processed.userId = null;
                    }
                  } else if (typeof nestedId === 'string' && nestedId.length === 24 && /^[0-9a-fA-F]{24}$/.test(nestedId)) {
                    processed.userId = nestedId;
                    console.log('✅ Extracted userId from _id (string):', processed.userId);
                  } else {
                    processed.userId = null;
                  }
                } else {
                  // Has keys but not _id - might be a different structure
                  console.warn('⚠️ userId is object but not ObjectId or populated user:', {
                    keys: keys,
                    constructor: processed.userId.constructor?.name,
                    make: processed.make,
                    model: processed.model
                  });
                  processed.userId = null;
          ...
          } else if (typeof processed.userId === 'string') {
            // Already a string, validate it
            if (processed.userId.length === 24 && /^[0-9a-fA-F]{24}$/.test(processed.userId)) {
              processed.userId = processed.userId;
            } else {
              console.warn('⚠️ Invalid userId string format:', processed.userId);
              processed.userId = null;
            }
          }
        } else {
          // userId is null or undefined - log for debugging
          console.warn('⚠️ userId is null/undefined for bike:', {
            _id: processed._id,
            make: processed.make,
            model: processed.model,
      ...
      console.log('✅ Pre-processed userId conversion:', bikeAds.length, 'ads');
      if (bikeAds.length > 0) {
        console.log('🔍 Sample pre-processed userId:', {
          userId: bikeAds[0].userId,
          userIdType: typeof bikeAds[0].userId,
          make: bikeAds[0].make,
          model: bikeAds[0].model
        });
      }
      
      // CRITICAL: Convert MongoDB ObjectIds to strings manually (lean() doesn't always do this)
      // This ensures _id and userId are always strings, not objects
      bikeAds = (bikeAds || []).map((ad) => {
        const converted = { ...ad };
        
        // Convert _id to string
        if (converted._id) {
          if (typeof converted._id === 'object') {
            // Try toString() first (MongoDB ObjectId)
            if (converted._id.toString && typeof converted._id.toString === 'function') {
              try {
                const idStr = converted._id.toString();
            ...
            // Try nested _id
            if (typeof converted._id === 'object' && converted._id._id) {
              converted._id = String(converted._id._id);
            }
            // Try $oid (BSON format)
            if (typeof converted._id === 'object' && converted._id.$oid) {
              converted._id = String(converted._id.$oid);
            }
          } else if (typeof converted._id === 'string') {
            // Already a string, keep it
            converted._id = converted._id;
          }
        }
        
        // Convert userId to string
        if (converted.userId) {
          if (typeof converted.userId === 'object') {
            const mongoose = require('mongoose');
            // CRITICAL: Check if it's an ObjectId instance FIRST (before checking keys)
            // ObjectId instances don't have enumerable keys, so Object.keys() returns []
            // Try toString() first - if it returns a valid 24-char hex string, it's an ObjectId
            let isObjectIdInstance = false;
            let userIdStr = null;
            
            if (converted.userId.toString && typeof converted.userId.toString === 'function') {
              try {
                userIdStr = converted.userId.toString();
                // Check if toString() returns a valid ObjectId string (24 hex chars)
                if (userIdStr && userIdStr !== '[object Object]' && userIdStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(userIdStr)) {
                  isObjectIdInstance = true;
                }
              } catch (e) {
                // toString() failed, not an ObjectId
              }
            }
            
            // Also check constructor name as backup
            if (!isObjectIdInstance && converted.userId.constructor) {
              const constructorName = converted.userId.constructor.name;
            ...
            // Also check instanceof
            if (!isObjectIdInstance && converted.userId instanceof mongoose.Types.ObjectId) {
              isObjectIdInstance = true;
              try {
                userIdStr = converted.userId.toString();
              } catch (e) {
            ...
            if (isObjectIdInstance && userIdStr) {
              // It's an ObjectId instance - convert to string directly
              converted.userId = userIdStr;
            } else {
              // Not an ObjectId instance - check for empty object or populated user object
              const keys = Object.keys(converted.userId);
              if (keys.length === 0) {
                converted.userId = null;
              } else {
                // Try nested _id (populated user object)
                if (converted.userId._id) {
                  const nestedId = converted.userId._id;
                  if (nestedId && nestedId.toString && typeof nestedId.toString === 'function') {
                    const idStr = nestedId.toString();
                    if (idStr && idStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(idStr)) {
                      converted.userId = idStr;
                    } else {
                      converted.userId = null;
                    }
                  } else if (typeof nestedId === 'string' && nestedId.length === 24 && /^[0-9a-fA-F]{24}$/.test(nestedId)) {
                    converted.userId = nestedId;
                  } else {
                    converted.userId = null;
                  }
                } else {
                  // Try $oid (BSON format)
                  if (converted.userId.$oid) {
                    converted.userId = String(converted.userId.$oid);
                  } else {
                    converted.userId = null;
                  }
                }
          ...
          } else if (typeof converted.userId === 'string') {
            // Already a string, validate it
            if (converted.userId.length === 24 && /^[0-9a-fA-F]{24}$/.test(converted.userId)) {
              converted.userId = converted.userId;
            } else {
              converted.userId = null;
            }
          }
      ...
      console.log('✅ Converted bike ads ObjectIds to strings:', bikeAds.length);
      if (bikeAds.length > 0) {
        console.log('🔍 Sample converted ad:', {
          _id: bikeAds[0]._id,
          _idType: typeof bikeAds[0]._id,
          userId: bikeAds[0].userId,
          userIdType: typeof bikeAds[0].userId,
          make: bikeAds[0].make,
          model: bikeAds[0].model
    ...
    // FIXED: Normalize userId and _id to strings (same as /rent_car/public)
    const toUserId = (v) => {
      if (!v) return null;
      
      const mongoose = require('mongoose');
      
      // CRITICAL: Check for empty object FIRST before toString()
      if (typeof v === 'object') {
        ...
        const isValidObjectId = mongoose.Types.ObjectId.isValid(v) ||
                                (v.constructor && 
                                 (v.constructor.name === 'ObjectId' || 
                                  v.constructor.name === 'Types.ObjectId' ||
                                  v instanceof mongoose.Types.ObjectId));
        
        // If it's a populated object (has _id property), extract _id
        if (v._id !== undefined) {
          const nestedId = v._id;
          if (nestedId && nestedId.toString && typeof nestedId.toString === 'function') {
            const idStr = nestedId.toString();
            if (idStr && idStr.length === 24 && /^[0-9a-fA-F]{24}$/.test(idStr)) {
              return idStr;
        ...
        // Try toString() for MongoDB ObjectId instances
        if (isValidObjectId || (v.toString && typeof v.toString === 'function')) {
          try {
            ...
            if (str && str !== '[object Object]' && str.length >= 10) {
              ...
              // Or any valid string ID
              if (str.length > 10 && str !== '[object Object]') {
                return str;
              }
            }
          } catch (e) {
        ...
        // Check if it's a MongoDB ObjectId constructor
        if (v.constructor && v.constructor.name === 'ObjectId') {
          try {
            const str = v.toString();
      ...
      if (typeof v === 'string') {
        if (v !== '[object Object]' && v.length >= 10) {
          // Validate MongoDB ObjectId format
          if (v.length === 24 && /^[0-9a-fA-F]{24}$/.test(v)) {
            return v;
          }
          // Or any valid string ID
          if (v.length > 10) {
    ...
    const toId = (v) => {
      ...
      if (typeof v === 'string') {
        if (v !== '[object Object]' && v.length >= 10) return v;
      ...
      if (typeof v === 'object') {
        ...
        // MongoDB ObjectId: Check if it has a constructor name or toString method
        // MongoDB ObjectId instances have toString() that returns 24-char hex string
        if (v.toString && typeof v.toString === 'function') {
          try {
            const str = v.toString();
            // MongoDB ObjectId toString() returns 24-char hex string (e.g., "507f1f77bcf86cd799439011")
            if (str && str !== '[object Object]' && str.length >= 10) {
              // Validate it looks like a MongoDB ObjectId (24 hex chars) or at least a valid ID
              if (str.length === 24 && /^[0-9a-fA-F]{24}$/.test(str)) {
                return str; // Valid MongoDB ObjectId
              }
              // Or any valid string ID (not "[object Object]")
              if (str.length > 10 && str !== '[object Object]') {
                return str;
              }
            }
        ...
        // Try nested _id property (for nested objects)
        if (v._id !== undefined) {
          const idStr = String(v._id);
          if (idStr && idStr !== '[object Object]' && idStr !== 'undefined' && idStr.length >= 10) {
            return idStr;
          }
        }
        // Try $oid (BSON format)
        if (v.$oid !== undefined) {
          const idStr = String(v.$oid);
          if (idStr && idStr !== '[object Object]' && idStr !== 'undefined' && idStr.length >= 10) {
            return idStr;
          }
        }
        ...
        // If object has constructor, check if it's MongoDB ObjectId
        if (v.constructor && v.constructor.name === 'ObjectId') {
          try {
            const str = v.toString();
      ...
      // Last resort: try String() but validate it's not "[object Object]"
      try {
        const fallback = String(v);
        if (fallback && fallback !== '[object Object]' && fallback !== 'undefined' && fallback !== 'null' && fallback.length >= 10) {
          return fallback;
        }
      } catch (e) {
    ...
    const normalized = (bikeAds || []).map((ad, index) => {
      const adCopy = { ...ad };
      
      // DEBUG: Log raw data for first bike ad
      if (index === 0) {
        console.log('🔍 RAW bike ad data (before normalization):', {
          _id: adCopy._id,
          _idType: typeof adCopy._id,
          _idConstructor: adCopy._id?.constructor?.name,
          _idKeys: adCopy._id && typeof adCopy._id === 'object' ? Object.keys(adCopy._id) : 'N/A',
          _idString: adCopy._id ? String(adCopy._id) : 'N/A',
          userId: adCopy.userId,
          userIdType: typeof adCopy.userId,
          userIdConstructor: adCopy.userId?.constructor?.name,
          userIdKeys: adCopy.userId && typeof adCopy.userId === 'object' ? Object.keys(adCopy.userId) : 'N/A',
          userIdString: adCopy.userId ? String(adCopy.userId) : 'N/A',
          make: adCopy.make,
          model: adCopy.model
        });
      }
      
      // FIXED: Normalize _id to string (always normalize, even if object)
      const normalizedId = toId(adCopy._id);
      if (normalizedId) {
        adCopy._id = normalizedId;
        adCopy.id = normalizedId; // Also set id field for consistency
      } else {
        // If normalization failed, log detailed info for debugging
        console.warn('⚠️ Failed to normalize bike _id:', {
          _id: adCopy._id,
          _idType: typeof adCopy._id,
          _idKeys: adCopy._id && typeof adCopy._id === 'object' ? Object.keys(adCopy._id) : 'N/A',
          _idString: adCopy._id ? String(adCopy._id) : 'N/A',
        ...
        // Try direct MongoDB ObjectId conversion
        if (adCopy._id && typeof adCopy._id === 'object') {
          try {
            // Check if it's a MongoDB ObjectId instance
            const isObjectId = mongoose.Types && mongoose.Types.ObjectId && mongoose.Types.ObjectId.isValid(adCopy._id);
            
            // Try calling toString() directly
            if (adCopy._id.toString && typeof adCopy._id.toString === 'function') {
              const directStr = adCopy._id.toString();
              if (directStr && directStr !== '[object Object]' && directStr.length >= 10) {
                adCopy._id = directStr;
                adCopy.id = directStr;
                console.log('✅ Fixed bike _id using direct toString():', directStr.substring(0, 10) + '...', isObjectId ? '(valid ObjectId)' : '');
              }
            }
            
            // If toString() didn't work, try converting to ObjectId and then string
            if (!adCopy._id || typeof adCopy._id === 'object') {
              try {
                if (mongoose.Types && mongoose.Types.ObjectId) {
                  const objId = new mongoose.Types.ObjectId(adCopy._id);
                  const objIdStr = objId.toString();
                  if (objIdStr && objIdStr.length === 24) {
                    adCopy._id = objIdStr;
                    adCopy.id = objIdStr;
                    console.log('✅ Fixed bike _id using ObjectId conversion:', objIdStr.substring(0, 10) + '...');
                  }
                }
              } catch (e) {
            ...
            console.error('⚠️ Error converting bike _id:', e);
          }
        }
      }
      
      // FIXED: Normalize userId to string (CRITICAL for seller info fetch)
      // Always normalize userId, even if it's an empty object (will become null)
      const originalUserId = adCopy.userId;
      
      // CRITICAL: Handle null/undefined userId properly
      if (originalUserId === null || originalUserId === undefined) {
        // userId is null/undefined - this is a problem, log it
        console.warn('⚠️ userId is null/undefined for bike ad:', {
          _id: adCopy._id,
          make: adCopy.make,
          model: adCopy.model,
          title: adCopy.title,
        ...
        // Try to normalize
        adCopy.userId = toUserId(adCopy.userId);
        
        // If normalization failed and original was an object, try direct conversion
        if (!adCopy.userId && typeof originalUserId === 'object') {
          // Try direct MongoDB ObjectId conversion
          try {
            if (mongoose.Types && mongoose.Types.ObjectId && mongoose.Types.ObjectId.isValid(originalUserId)) {
              const objId = new mongoose.Types.ObjectId(originalUserId);
              adCopy.userId = objId.toString();
              console.log('✅ Fixed userId using direct ObjectId conversion:', adCopy.userId.substring(0, 10) + '...');
            }
          } catch (e) {
            console.error('❌ Failed to convert userId ObjectId:', e);
          }
        }
      }
      
      // DEBUG: Log userId normalization for first bike ad
      if (index === 0) {
        console.log('🔍 userId normalization:', {
          original: originalUserId,
          originalType: typeof originalUserId,
          originalConstructor: originalUserId?.constructor?.name,
          originalKeys: originalUserId && typeof originalUserId === 'object' ? Object.keys(originalUserId) : 'N/A',
          normalized: adCopy.userId,
          normalizedType: typeof adCopy.userId,
          make: adCopy.make,
          model: adCopy.model
        });
      }
      
      // Also normalize sellerId and postedBy if they exist
      if (adCopy.sellerId) {
        adCopy.sellerId = toUserId(adCopy.sellerId);
      }
      if (adCopy.postedBy) {
        adCopy.postedBy = toUserId(adCopy.postedBy);
      }
      
      // Serialize dates
      adCopy.dateAdded = toISO(adCopy.dateAdded) || toISO(adCopy.approvedAt) || adCopy.dateAdded || new Date().toISOString();
      if (adCopy.approvedAt) adCopy.approvedAt = toISO(adCopy.approvedAt);
    ...
    clearTimeout(timeout);
    const queryTime = Date.now() - startTime;
    console.log(`✅ /bike_ads/public: Found ${normalized.length} ads in ${queryTime}ms`);
    
    // Debug: Log sample bike with userId info
    if (normalized.length > 0) {
      const sample = normalized[0];
      console.log(`🔍 Sample bike ad:`, {
        id: sample._id,
        idType: typeof sample._id,
        make: sample.make,
        model: sample.model,
        ...
        originalId: bikeAds[0]?._id,
        originalIdType: typeof bikeAds[0]?._id,
        originalUserId: bikeAds[0]?.userId,
        originalUserIdType: typeof bikeAds[0]?.userId
      });
    }
...
app.post("/rent_car", uploadRentCarImages, multerErrorHandler, async (req, res) => {
  try {
    ...
    // Car on Rent: 1st ad FREE and active; 2nd+ ad requires payment receipt → pending until admin approves
    const existingCount = await Rent_Car.countDocuments({ userId: new mongoose.Types.ObjectId(userId), isDeleted: { $ne: true } });
    const isFirstAd = existingCount === 0;
    const paymentReceiptPath = getPath(files.paymentReceipt);
    if (!isFirstAd && !paymentReceiptPath) {
      return res.status(400).json({
        success: false,
        message: "Payment receipt is required for additional Car on Rent ads. Please upload your payment receipt."
      });
    }
    const doc = {
      userId: new mongoose.Types.ObjectId(userId),
    ...
    if (isFirstAd) {
      ...
      doc.isPaidAd = true;
      if (b.paymentAmount) doc.paymentAmount = Number(b.paymentAmount);
    }
    const ad = new Rent_Car(doc);
    await ad.save();
    res.status(201).json({
      success: true,
      message: isFirstAd ? "Ad posted successfully. Your first Car on Rent ad is now live." : "Ad submitted. It will go live after admin verifies your payment receipt.",
      data: { id: ad._id.toString(), isFirstAd, paymentStatus: doc.paymentStatus }
    });
  } catch (error) {
...
// Show: 1st free ad (paymentStatus 'free') + ads approved by admin (paymentStatus 'verified')
app.get("/rent_car/public", securityMiddleware.sanitizePublicResponse, async (req, res) => {
  try {
    const rentCars = await Rent_Car.find({
      isDeleted: { $ne: true },
      isActive: true,
      paymentStatus: { $nin: ['pending', 'rejected'] }
    })
      .sort({ dateAdded: -1 })
      .lean();
    // Serialize dateAdded to ISO and _id/userId to string so app works correctly
    const toISO = (v) => { if (!v) return null; try { return (v instanceof Date) ? v.toISOString() : new Date(v).toISOString(); } catch (e) { return null; } };
    const toUserId = (v) => {
      if (!v) return null;
    ...
    const withDates = (rentCars || []).map((ad) => {
      const adCopy = { ...ad };
      if (adCopy._id && typeof adCopy._id === 'object') {
        adCopy._id = adCopy._id.toString ? adCopy._id.toString() : String(adCopy._id);
      }
      ...
      // Ensure isActive is sent (default true for rental so Car on Rent list shows new ads)
      if (adCopy.isActive === undefined) adCopy.isActive = true;
      adCopy.adSource = 'rent_car';
      adCopy.modelType = 'rentcar';
      normalizeAdImagePaths(adCopy);
      return adCopy;
    });
    res.json(withDates);
  } catch (error) {
...
// Admin: Approve Car on Rent ad after verifying payment receipt (sets active + shows on public)
app.patch("/admin/rent_car/:id/approve", authenticateToken, async (req, res) => {
  try {
    const userType = String(req.userType || '').toLowerCase();
    if (userType !== 'admin' && userType !== 'superadmin') {
    ...
    const ad = await Rent_Car.findById(id);
    if (!ad) return res.status(404).json({ success: false, message: "Car on Rent ad not found." });
    if (ad.isDeleted) return res.status(400).json({ success: false, message: "Ad is deleted." });
    const updated = await Rent_Car.findByIdAndUpdate(
      id,
      { paymentStatus: 'verified', isActive: true, approvedAt: new Date() },
      { new: true }
    ).lean();
    if (process.env.NODE_ENV !== 'production') console.log('✅ Rent car ad', id, 'approved by', req.userId);
    res.json({ success: true, message: "Car on Rent ad approved and now live.", ad: updated });
  } catch (error) {
    if (process.env.NODE_ENV !== 'production') console.error('PATCH admin/rent_car approve error:', error);
    res.status(500).json({ success: false, message: "Error approving ad.", error: process.env.NODE_ENV !== 'production' ? error.message : undefined });
  }
});
...
const inspectionRoutes = reqRoot('routes/inspection');
...
app.get("/dealer_packages/:packageId", async (req, res) => {
  try {
    ...
    const DealerPackage = reqRoot('models/DealerPackage');
